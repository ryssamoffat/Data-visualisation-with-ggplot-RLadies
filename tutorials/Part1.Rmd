...
pacman::p_load(here, # helps keep paths short by starting them from the project
               ...
               devtools) # for downloading packages in development
...
personality_traits %>%
  ...
  geom_point(color = "maroon") # try "red", "cornflowerblue", "maroon"
...
personality_traits %>%
  ggplot(aes(x = extraversion_z, y = age))+
  geom_point(color = "#dd4e2c", size = 3, shape = 23) # Rladies purple #89398a
...
personality_traits %>%
  ggplot(aes(x = extraversion_z, y = age)) +
  geom_jitter(color = "purple", fill = "yellow", alpha = 0.5, shape = 21, size = 3)


# scatter plot with trend line
# try changing color, fill, method (loess, lm)
personality_traits %>%
  ggplot(aes(x = extraversion_z, y = cati_z)) +
...
personality_traits %>%
  ...
  geom_jitter(color = "purple", fill = "salmon", shape = 25, size = 4)+
  geom_vline(xintercept = 1, color = "maroon", linetype = 4,
...
personality_traits1 <- personality_traits %>%
  mutate(agegroup = as.factor(case_when(age < 25 ~ "<25",
                                       age > 24 & age < 40 ~ "25-39", 
                                       age > 39 ~ "40+")))
...
Back to basic `ggplot2` themes and customisation! Play around and change the themes by uncommenting them one at at time.
...
theme_set(theme_light()+
            theme(
              ...
              strip.text = element_text(size=rel(1), colour = 'white', face = "italic"),
              legend.text = element_text(size=rel(1)),
              legend.title = element_text(size=rel(1),face="bold"),
...
theme_set(theme_light()+
            theme(
              ...
              panel.grid.major.y = element_line(color = "red", linewidth = 1),
              panel.grid.minor = element_line(color = "orange", linetype = "dashed", linewidth = 1))) 
